#--------------------------------------
#Generate reference trajectory for NMPC
#--------------------------------------

import numpy as np
import math
#import tf.transformations as tf


# Parameters
sample_time = 0.05             # seconds
duration = 240                   # seconds

r = 10
v = 1.5

x0 = 0                       
y0 = 0
z0 = -5

# trajectory
traj = np.zeros((int(duration/sample_time+1),20)) # x y z phi theta psi u v w p q r u1 u2 u3 u4 u5 u6
t = np.arange(0,duration,sample_time)
t = np.append(t, duration)

traj[:,0] = 0  # x
traj[:,1] = 0   # y
traj[:,2] = z0                      # z
traj[:,3] = 0  #0.5*np.pi*np.sin(t*0.1/1)                       # phi
traj[:,4] = 0                       # theta
traj[:,5] = 3        # psi
traj[:,6] = 0                       # u
traj[:,7] = 0                       # v
traj[:,8] = 0                       # w
traj[:,9] = 0                       # p
traj[:,10] = 0                      # q
traj[:,11] = 0                      # r
traj[:,12] = 0                      # u1
traj[:,13] = 0                      # u2
traj[:,14] = 0                      # u3
traj[:,15] = 0                      # u4
traj[:,16] = 0                      # u5
traj[:,17] = 0                      # u6
traj[:,18] = 0                      # u7
traj[:,19] = 0                      # u8
'''
for i in range(0,int(duration/sample_time+1)):
    if np.sin(traj[i,5])>=0:
        traj[i,5] = traj[i,5]%np.pi
    else:
        traj[i,5] = -np.pi + traj[i,5]%np.pi
    #traj[i,5] = np.sin(traj[i,5])
'''
sta = np.zeros((int(50/sample_time+1),20))
sta[:,0] = 0
sta[:,2] = z0
sta[:,5] = -1.570796
traj2 = np.concatenate((sta,traj))

# write to txt
np.savetxt('pos_test.txt',traj,fmt='%f')
