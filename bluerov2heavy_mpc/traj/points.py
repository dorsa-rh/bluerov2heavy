...
step_interval = 5
wait_time = 10
points_matrix = np.array([[0.0,0.0,-15.0],[2.87,0,-15],[2.87, -8.2,  -15.0],[-4.63,-8.2,  -15.0],[-4.63,-20.2, -15.0],[-7.63,-20.2, -15.0]])
# --- Waypoints (x, y, z) ---
'''waypoints = np.array([
    (0.0,   0.0,  -15.0),
    (2.87,  0.0,  -15.0),
...
traj = np.zeros((int(duration/sample_time+1),18)) # x y z phi theta psi u v w p q r u1 u2 u3 u4
...
n_sta = int(wait_time / sample_time) + 1
sta = np.zeros((n_sta, 18))
sta[:, 0] = points_matrix[0, 0]
sta[:, 1] = points_matrix[0, 1]
sta[:, 2] = points_matrix[0, 2]
sta[:, 5] = traj[0, 5]  # yaw

traj_full = np.vstack((sta, traj))
# write to txt
np.savetxt('points.txt',traj_full,fmt='%f')